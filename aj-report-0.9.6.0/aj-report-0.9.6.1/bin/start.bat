...
set JAVA_HOME1=C:\Program Files\Java\jdk1.8.0_201
IF "%JAVA_HOME1%" == "" (
	goto END
) ELSE (
    goto START
...
:START
	...
	rem 正式启动
	"%JAVA_HOME1%"\bin\java %JAVA_OPTS% -Xbootclasspath/a:%LIB_JARS% -jar -Dspring.config.location=%CONF_YML% %BIN_DIR%\lib\%BOOT_JAR%
	pause
